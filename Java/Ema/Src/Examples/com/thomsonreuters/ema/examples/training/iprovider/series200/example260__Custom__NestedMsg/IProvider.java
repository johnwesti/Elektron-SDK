...
class AppClient implements OmmProviderClient
{
	...
	public void processCustomDomainRequest(ReqMsg reqMsg, OmmProviderEvent event)
	{
		if(itemHandle != 0)
		{
		...
		event.provider().submit( EmaFactory.createGenericMsg().name("GenericMsg").payload(
				EmaFactory.createRefreshMsg().name(reqMsg.name()).serviceName(reqMsg.serviceName())
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			for(int i = 0; i < 60; i++)
			{
			   ...
			   provider.submit(genericMsg.clear().name("GenericMsg").payload(
