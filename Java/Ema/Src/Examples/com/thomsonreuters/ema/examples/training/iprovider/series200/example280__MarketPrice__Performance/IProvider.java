...
package com.thomsonreuters.ema.examples.training.iprovider.series200.example280__MarketPrice__Performance;
...
class AppClient implements OmmProviderClient
{
	...
	public void processLoginRequest(ReqMsg reqMsg, OmmProviderEvent event)
	{
	...
	public void processMarketPriceRequest(ReqMsg reqMsg, OmmProviderEvent event)
	{
	...
	public void processInvalidItemRequest(ReqMsg reqMsg, OmmProviderEvent event)
	{
		event.provider().submit(EmaFactory.createStatusMsg().name(reqMsg.name()).serviceName(reqMsg.serviceName())
				.domainType(reqMsg.domainType())
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			{	
				...
				for(int index = 0; index < appClient.numberOfRequestItems; index++ )
				{	
					...
					provider.submit(updateMsg.clear().payload(appClient.fieldList )
						, reqInfo.handle());
				...
				while (System.currentTimeMillis() - startUpdateTime < 1000);;
				
				float timeSpent = (float)(endUpdateTime - startUpdateTime) / (float)1000;
				
				System.out.println("update count = " + updateMsgCount +
						"\nupdate rate = " + (float)updateMsgCount / timeSpent + " update per sec");
