...
- after submitting refresh and 10 updates,
  sending a directory update with service state change (using the domain Directory helper class)
...
class AppClient implements OmmProviderClient
{
	...
	void processUpdateServiceStatus( OmmProvider provider)
	{
		...
		Dir.State state = DomainFactory.Directory.createState();
		state.serviceState( Dir.ServiceState.SERVICE_DOWN );
		
		Dir.Service service = DomainFactory.Directory.createService();
		service.info( info ).state( state );
